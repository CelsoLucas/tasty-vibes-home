...
WITH restaurant_ids AS (
  SELECT id FROM public.restaurants LIMIT 5
),
sample_users AS (
  SELECT id FROM auth.users LIMIT 3
)
INSERT INTO public.reviews (restaurant_id, user_id, rating, comment, review_images) VALUES
((SELECT id FROM restaurant_ids OFFSET 0 LIMIT 1), (SELECT id FROM sample_users OFFSET 0 LIMIT 1), 5, 'Experiência incrível! A comida estava deliciosa e o atendimento impecável. Recomendo muito!', ARRAY['/src/assets/restaurant-1.jpg']),
((SELECT id FROM restaurant_ids OFFSET 0 LIMIT 1), (SELECT id FROM sample_users OFFSET 1 LIMIT 1), 4, 'Ambiente muito agradável, preços justos. A comida chegou quentinha e saborosa.', ARRAY[]::TEXT[]),
((SELECT id FROM restaurant_ids OFFSET 1 LIMIT 1), (SELECT id FROM sample_users OFFSET 0 LIMIT 1), 5, 'Melhor steakhouse da cidade! Carne no ponto perfeito e atendimento de primeira.', ARRAY['/src/assets/restaurant-2.jpg', '/src/assets/restaurant-3.jpg']),
((SELECT id FROM restaurant_ids OFFSET 2 LIMIT 1), (SELECT id FROM sample_users OFFSET 2 LIMIT 1), 4, 'Comida árabe autêntica e saborosa. O ambiente é acolhedor.', ARRAY[]);